...
import { dequeue, getQueue, getQueueSummary, markRetry, resetRetry, RETRY_EXHAUSTED_AT } from "@/lib/offlineQueue";
...
export function OfflineSyncProvider({ children }: { children: React.ReactNode }) {
    ...
    try {
      ...
      for (const item of queue) {
        ...
        try {
          ...
          latestError = e instanceof Error ? e.message : "동기화 실패";
  ...
  const value = useMemo<OfflineSyncContextValue>(() => ({
    ...
    nextRetryAt,
    syncNow,
  }), [pendingCount, blockedCount, exhaustedCount, syncing, lastSyncedAt, lastError, nextRetryAt, syncNow]);
