...
const KakaoMap = dynamicImport(() => import("@/components/KakaoMap"), {
    ssr: false,
});
...
export default function Home() {
    ...
    const [selectedPreset, setSelectedPreset] = useState<IntervalPreset>(
        INTERVAL_PRESETS[0],
    );
    const [search, setSearch] = useState<{
        open: boolean;
        query: string;
        results: kakao.maps.services.PlaceResult[];
        pointMode: "start" | "end";
        loading: boolean;
        preview: kakao.maps.services.PlaceResult | null;
    }>({
        open: false,
        query: "",
        results: [],
    ...
    const [route, setRoute] = useState<{
        info: {path: LatLng[]; distanceM: number; durationS: number} | null;
        loading: boolean;
    ...
    useEffect(() => {
        if (!startPoint) {
            setAddresses((prev) => ({...prev, start: ""}));
            return;
        }
        if (startAddrOverride.current) {
            startAddrOverride.current = null;
            return;
        }
        reverseGeocode(startPoint, (v) =>
            setAddresses((prev) => ({...prev, start: v})),
        );
    }, [startPoint]);

    useEffect(() => {
        if (!endPoint) {
            setAddresses((prev) => ({...prev, end: ""}));
            return;
        }
        if (endAddrOverride.current) {
            endAddrOverride.current = null;
            return;
        }
        reverseGeocode(endPoint, (v) =>
            setAddresses((prev) => ({...prev, end: v})),
        );
    }, [endPoint]);
    ...
    useEffect(() => {
        ...
        fetch(
            ...
            .then((info) => {
                if (!cancelled) setRoute((prev) => ({...prev, info}));
            })
            .catch(() => {
                if (!cancelled) setRoute((prev) => ({...prev, info: null}));
            })
            .finally(() => {
                if (!cancelled) setRoute((prev) => ({...prev, loading: false}));
            });
    ...
    function handleStartPointChange(latlng: LatLng) {
        setStartPoint(latlng);
    }
    function handleEndPointChange(latlng: LatLng) {
        setEndPoint(latlng);
    }
        ...
        ps.keywordSearch(
            ...
            (results, status) => {
                setSearch((prev) => ({
                    ...
                    loading: false,
                    results:
                        status === kakao.maps.services.Status.OK ? results : [],
                }));
            },
            {size: 10},
        );
    ...
    const guide =
        pageMode === "goal"
            ? {
                  text: `목표: ${goal.distance} km · 위치 자동 설정`,
                  color: accentVar,
              }
            : !startPoint
    ...
    return (
        <main className="relative w-full h-dvh overflow-hidden">
            <KakaoMap
                ...
                onStartPointChange={
                    pageMode === "map" ? handleStartPointChange : undefined
                }
                onEndPointChange={
                    pageMode === "map" ? handleEndPointChange : undefined
                }
                ...
                previewPoint={
                    search.preview
                        ? {
                              lat: parseFloat(search.preview.y),
                              lng: parseFloat(search.preview.x),
                          }
                        : null
                }
            ...
            >
                ...
                <div className="flex items-center gap-2">
                    <button
                        onClick={() =>
                            openSearchFor(mode === "end" ? "end" : "start")
                        }
                        ...
                        >
                            <div className="flex items-center gap-2">
                                ...
                                <div className="min-w-0">
                                    ...
                                    >
                                        {search.preview.road_address_name ||
                                            search.preview.address_name}
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    >
                        ...
                        >
                            ...
                            {(search.pointMode === "start"
                                ...
                                : addresses.end) && (
                                <span
                                    className="ml-1 text-[11px] truncate flex-1 text-right"
                                    style={{color: "var(--c-text-3)"}}
                                >
                                    {search.pointMode === "start"
                                        ? addresses.start
            ...
            >
                ...
                <div
                    ...
                    onClick={() => {
                        if (!didDrag.current) setSheetOpen((o) => !o);
                ...
                <div className="px-4 space-y-3">
                    ...
                    {/* 활동 유형 */}
                    <div
                        className="card flex gap-2 p-1 rounded-2xl"
                        style={{background: "var(--c-elevated)"}}
                    >
        ...
        >
            <div className="flex items-center gap-1.5 mb-1">
                <span
                    ...
                    style={{
                        background: isStart ? "var(--c-toss-blue)" : "#f59e0b",
                    }}
                />
                <span
            ...
            {point ? (
                <>
                    ...
                    <p
                        className="text-[10px] num truncate mt-0.5"
                        style={{color: "var(--c-text-3)"}}
                    >
